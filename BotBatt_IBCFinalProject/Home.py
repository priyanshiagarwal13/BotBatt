...
# page settings
st.set_page_config(page_title="BotBatt", layout="wide")
st.header("BotBatt")
st.text(
    "BotBatt is a lightweight data analysis app powered by LLM, showcasing how LLM can revolutionize the future"
    "of data analysis."
)
...
class AgentWrapper:
    ...
    def chat(self, prompt):
        if self.agent is None:
            ...
            # Preprocess the prompt to remove special characters
            cleaned_prompt = re.sub(r'[^\w\s]', '', prompt)
            return self.agent.chat(cleaned_prompt)
    ...
    def set_file_data(self, df):
        ...
        if llm is not None:
            ...
            config = Config(
                ...
                custom_prompts={"generate_python_code": get_prompt_template()},
                enable_cache=False,
                verbose=True
            )
...
def get_agent(agent_id) -> AgentWrapper:
    ...
    return agent
def set_file_data(self, df):
    # Check if 'Unnamed: 12' column exists and has type 'object'
    if 'Unnamed: 12' in df.columns and df['Unnamed: 12'].dtype == 'object':
        # Convert the column to bytes if possible, otherwise handle the conversion based on your data
        df['Unnamed: 12'] = df['Unnamed: 12'].apply(lambda x: str(x).encode() if isinstance(x, int) else x)
...
with st.sidebar:
    ...
    if option == "OpenAI":
        ...
        access_token = st.text_input(
            "Access Token", st.session_state.access_token, type="password", placeholder="Access token"
        )
    elif option == "Baidu/Qianfan-Ernie-Bot":
        client_id = st.text_input("Client ID", st.session_state.client_id, placeholder="Client ID")
        client_secret = st.text_input("Client Secret", st.session_state.client_secret, type="password", placeholder="Client Secret")
    elif option == "Ollama":
        ollama_model = st.selectbox(
            "Choose Ollama Model",
            ["starcoder:7b", "codellama:7b-instruct-q8_0", "zephyr:7b-alpha-q8_0"]
        )
        ollama_base_url = st.text_input("Ollama BaseURL", st.session_state.ollama_base_url, placeholder="http://localhost:11434")
